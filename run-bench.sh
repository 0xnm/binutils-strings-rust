...
# strings doesn't support it as of 2.37, using the version build manually. Also it crashes on big (target/debug/strings) file, so using a smaller file.
# Don't trust the result a lot.
hyperfine -w 5 'target/release/strings -u escape target/release/strings' '~/binutils-gdb/binutils/strings -Ue target/release/strings' --export-markdown $BENCH_UNICODE_FILE_MODE_FILE
...
TIME_OUTPUT_GDB_UNICODE_FILE_MODE=$(${TIME_RUN_COMMAND} ~/binutils-gdb/binutils/strings -Ue target/release/strings 2>&1 >/dev/null | ${EXTRACT_STATS_COMMAND})
